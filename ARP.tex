...
\section{Definizione e prime propriet\`a}
...
\section{Gruppo di simmetrie e regolarit\`a}
...
\end{proof}
\subsubsection{Generatori scelti per $\Gamma$}
...
\subsection{Definizioni e criteri}
...
\end{proof}
\subsection{Criteri di politopicit\`a}
...
\currentprefix{prop:QuotientRegularIsPoly}
...
\end{prop}
\begin{proof}
Per dimostrare questo criterio utilizzeremo il criterio gi\`a visto, \ref{prop:QuotientIsPolitope}. Cominciamo quindi col mostrare che la
suriettivit\`a di $\gamma$ per qualunque scelta di $(F,G,\Omega)$ \`e equivalente a \localref{1}. Sia $\Phi$ la bandiera base di $\p$ scelta.
Per la regolarit\`a di $\p$ ogni catena \`e coniugata tramite elementi di $\Gamma_\p$ ad un sottoinsieme di $\Phi$ pertanto, qualunque
sia la terna $(F,G,\Omega)$ con $F,G\in\Omega$ e $\Omega$ catena di $\p$ di tipo $(-1,\cdots,f,g,\cdots,n)$ (quindi come dalla
definizione di $\gamma$), esiste un $\phi\in\Gamma_\p$ tale che $F=F_f^\phi$, $G=F_g^\phi$ e
$\Omega=\left\{F_i^\phi\mid i\notin\{f+1,\dots,g-1\}\right\}$, quindi possiamo scrivere
\begin{equation*}
\gamma:\frac{F_g^\phi/F_f^\phi}{\Sigma_\Omega}\longrightarrow\frac{(F_g^\phi)^\Sigma}{(F_f^\phi)^\Sigma}
\end{equation*}
Sia quindi $f<k<g$. Le facce di rango $k$ in $\p/\Sigma$ incidenti con $(F_g^\phi)^\Sigma$ sono precisamente quelle del tipo
$(F_k^{\alpha_1\phi})^\Sigma$ con $\alpha_1\in\Gamma_{-1\rightarrow g}$, invece quelle incidenti con $(F_f^\phi)^\Sigma$ sono
precisamente quelle del tipo$(F_k^{\alpha_2\phi})^\Sigma$ con $\alpha_2\in\Gamma_{f\rightarrow n}$.\\
Da ci\`o abbiamo che le $k$-facce della sezione $F_g^{\phi\Sigma}/F_f^{\phi\Sigma}$ possono essere scritte in ambo i modi (e sono
caratterizzate da questa propriet\`a) e, in tal caso quindi si avr\`a
\begin{equation*}
F_k^{\alpha_1\phi\Sigma}=F_k^{\alpha_2\phi\Sigma}
\end{equation*}
e quindi $\alpha_1\phi\in\Gamma_k\alpha_2\phi\Sigma$.\\
Pertanto, assumendo \localref{1}, abbiamo che esiste un $\alpha_3\in\Gamma_{f\rightarrow g}$ tale che $\alpha_1\phi\Gamma_k\alpha_3\phi\Sigma$
e $F_k^{\alpha_1\phi\Sigma}=F_k^{\alpha_3\phi\Sigma}$, per\`o $F_k^{\alpha_3\phi}\in F_g^\phi/F_f^\phi$, cio\`e $\gamma$ \`e suriettiva.\\
Viceversa, supponendo $\gamma$ suriettiva per ogni scelta di $(F,G,\Omega)$ come sopra, allora una $k$-faccia
$F_k^{\alpha_1\phi\Sigma}=F_k^{\alpha_2\phi\Sigma}$ in $F_g^{\phi\Sigma}/F_f^{\phi\Sigma}$ \`e della forma $F_k^{\alpha_3\phi\Sigma}$
con $\alpha_3\in\Gamma_{f\rightarrow g}$.\\
Pertanto, se $\beta_1,\beta_2\in\Gamma_k$, $\alpha_1\in\Gamma_{-1\rightarrow g}$ e $\alpha_2\in\Gamma_{f\rightarrow n}$ sono tali che
$\beta_1\alpha_1\phi\Sigma=\beta_2\alpha_2\phi\Sigma$ allora $F_k^{\alpha_1\phi\Sigma}=F_k^{\alpha_2\phi\Sigma}=F_k^{\alpha_3\phi\Sigma}$
per un opportuno $\alpha_3\in\Gamma_{f\rightarrow g}$ pertanto
$\beta_1\alpha_1\phi\Sigma\subseteq\Gamma_k\Gamma_{f\rightarrow g}\phi\Sigma$ che prova un'inclusione di \localref{1}, l'altra \`e
banale.\\
Adesso mostriamo che il punto \localref{2} \`e invece equivalente alla seconda condizione del criterio \ref{prop:QuotientIsPolitope}.
Osserviamo intanto che $\Sigma$ non contiene nessun elemento coniugato a $\rho_i$ se e solo se la proiezione relativa a $\Sigma$ non
identifica nessuna coppia di bandiere adiacenti di $\p$, in quanto se $\phi\in\Gamma_\p$ allora
$(\Phi\phi)^i\Sigma=(\Phi\phi)(\rho_i^\phi)\Sigma$. Pertanto, se vale \localref{2} allora le orbite di bandiere adiacenti di $\p$ devono
essere rimanere distinte, pertanto le mappe $\beta$ e $\gamma$ di cui al \ref{prop:QuotientIsPolitope} devono essere iniettive.\\
Viceversa, supponendo che \localref{2} non valga per un $k$ e un $\phi$, cio\`e che $\rho_k\phi^{-1}\in\Gamma_k\phi^{-1}\Sigma$.
Allora $F_k^{\rho_k\phi^{-1}\Sigma}=F_k^{\phi^{-1}\Sigma}$ cos\`i che le due facce $F_k^{\rho_k\phi^{-1}}$ e $F_k^{\phi^{-1}}$
della sezione $F_{k+1}^{\phi^{-1}}/F_{k-1}^{\phi^{-1}}$ vengono identificate tramite $\pi=\beta\gamma$. Pertanto $beta$ e $\gamma$
non possono essere (entrambe) iniettive. Questo mostra che il presente criterio \`e in effetti equivalente al precedente
\ref{prop:QuotientIsPolitope}.
\end{proof}
\include{esempi} 
