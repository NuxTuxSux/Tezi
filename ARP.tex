...
\currentprefix{prop:RegularSections}
\begin{prop}
...
generatori scelti. Allora, se $\sigma\in\Gamma(\p)$ \`e tale che $\Phi^\sigma=\Psi$, i due sistemi di generatori scelti saranno coniugati
da $\sigma$, ossia
\begin{equation*}
...
Sia $\p$ un politopo regolare, $\Phi$ la bandiera di base scelta. Allora per ogni $J\subseteq \Phi$
\begin{equation*}
...
\end{example}
\begin{proof}
Per il teorema \ref{teo:Generators}, abbiamo che $\Gamma=\langle x,y\rangle$. Inoltre $\Gamma$ agisce transitivamente sull'insieme dei
lati di $P$, $P_1$. Se $l\in P_1$, per la proposizione \ref{prop:DistSubgroups} abbiamo che $\Gamma_{l_0}=\Gamma(l_0/F_{-1})\cong\langle x\rangle$
pertanto, passando alle cardinalit\`a:
\begin{equation*}
\left|P_1\right|=\left|l_0^\Gamma\right|=\left[\Gamma:\Gamma_{l_0}\right]=\frac{\left|\Gamma\right|}{\left|\Gamma_{l_0}\right|}
\end{equation*}
Essendo $\Gamma_{l_0}\cong\Gamma(l_0/F_{-1})$...[!CONTINUARE]
\end{proof}
\end{document}
